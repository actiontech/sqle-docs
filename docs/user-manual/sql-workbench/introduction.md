...
SQL工作台为SQLE平台提供数据库可视化操作能力，方便开发或DBA直接在SQLE中操作数据库。
...
| 方案 | 说明 | 状态 |
|------|------|------|
| CloudBeaver | 开源Web数据库管理工具 | 当前支持，后续将逐步替换 |
| 新版工作台 | 新一代SQL工作台 | 推荐使用 |

:::tip
建议新部署环境使用新版工作台方案，已有环境可继续使用 CloudBeaver。
:::
...
根据您选择的工作台方案，完成相应的安装配置：
- CloudBeaver方案：参考 [CloudBeaver部署配置](cloudbeaver/intro.md)
- 新版工作台方案：参考 [新版工作台部署](odc/deploy.md)

## 使用场景

当业务人员需要对数据库进行数据检索时，一些查询SQL可能会造成数据库性能问题甚至夯死。但在传统的使用场景中，DBA很难对业务人员的行为进行追踪和管控，无法限制有问题的SQL在生产环境执行。

因而SQLE平台集成数据库可视化管理工具，满足在线SQL查询需求的同时，进行业务人员的操作权限控制，并对业务人员的SQL进行审计，拒绝不合规的SQL运行。
...
#### 步骤三 登录SQL工作台
切换成员A账号后，点击SQL工作台进入，SQLE将会自动使用当前SQLE用户登录。
...
* 可以看到有实例testread（因为在这个实例上具备SQL查询权限）；
  * 但由于连接账号只有读取权限，所以无法通过SQL工作台进行数据变更；
